...
tests =
  testGroup
    ...
    [ testCase "v1 default payload decodes"
        ...
        decodeConfig 0b000000
        @?= Right (Config Controller X1 Sck20 AlertPin)
    , testCase "alert_source=io1 accepted"
        $ decodeConfig 0b000001
        @?= Right (Config Controller X1 Sck20 AlertIo1)
    , testCase "target role rejected in v1"
        $ decodeConfig 0b100000
        @?= Left UnsupportedRole
    , testCase "dual I/O rejected in v1"
        $ decodeConfig 0b001000
        @?= Left UnsupportedIoMode
    , testCase "33 MHz rejected in v1"
        $ decodeConfig 0b000010
        @?= Left UnsupportedSck
    ]
