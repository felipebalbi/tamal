...
**Background:** tamal's `addi`/`load_imm` sign-extend an **11-bit** immediate (`[-1024,1023]`), not RISC-V's 12-bit. So `li` = `load_imm` (fits signed-11), else `lui` (top 20 bits) followed by a **greedy chain of `addi`s** summing to the residual. The residual lands in `[-2048, 2047]`; since one `addi` covers `[-1024,1023]`, up to **3** `addi`s are needed (residual `2047` = `1023+1023+1`), giving a worst case of `lui + 3 addi` = **4** instructions. This is fully general.
...
pub(crate) fn tile_li(rd: Reg, value: i32) -> Vec<Instr> {
    ...
    let mut resid = low12;
    if resid > 2047 {
        resid -= 4096; // now in [-2048, -1]
        hi = hi.wrapping_add(1) & 0xF_FFFF;
    }
    let mut out = vec![Instr::Lui(rd, Imm20::new(hi).expect("20-bit pattern fits"))];
    while resid != 0 {
