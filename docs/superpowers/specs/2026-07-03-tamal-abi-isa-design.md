...
(checked), `bits(&self) -> repr`, and a **private infallible**
`from_bits(masked) -> Self` used by `decode` (fields are already width-masked by
`split_word`). A `TryFrom<repr>` impl was considered but intentionally **omitted**:
its natural `Error = ()` trips `clippy::result_unit_err` under the crate's
`-D warnings` gate, and `new` already covers checked construction. If a future
consumer needs `TryFrom`, add it with a real (non-unit) error type.
